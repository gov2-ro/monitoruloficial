...
- [ ] **Revisit `max_consecutive_failures` / safe batch size using `requests_made` data** — `fetch_p3+.py` now logs a `requests_made` counter per run (2026-07-07). Once a few weeks of cron cycles have accumulated in the `runs` table, check `stats.py` for whether failures correlate with request volume, time-of-day, or run duration, and use that to size cron batches or the failure threshold with actual evidence instead of a guess.
